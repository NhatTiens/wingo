...
def atomic_csv(path, header, rows):
    ...
    try:
        ...
        # Windows/OneDrive ??i khi kh?a CSV trong th?i gian r?t ng?n.
        # Export CSV ch? ph?c v? dashboard, kh?ng ???c ph?p l?m ch?t writer.
        last_exc = None
        for _ in range(10):
            try:
                os.replace(tmp,path)
        ...
        print(f"[CSV WARN] Kh?ng th? c?p nh?t {path}: {last_exc}")
        print("[CSV WARN] B? qua export v?ng n?y; d? li?u trong SQLite v?n an to?n.")
...
def main():
    ...
    ap.add_argument("--paper-market",choices=["off","size","color","number","best"],default="off")
                ...
                if ctl and int(ctl.get("tool_running") or 0) and int(ctl.get("real_bet_armed") or 0):
                    ...
                    if li["issue"]==info["issue"]:
                        ...
                        if bi["issue"]==info["issue"] and bi["second"]<=bet_sec and bi["second"]>int(bi.get("close",3)):
                            def current_issue_guard():
                                current=timing(fetch(s))
                                if (current["issue"]!=info["issue"]
                                        or current["second"]<=int(current["close"])+1):
                                    raise RealBetError("Kỳ cược đã đổi hoặc quá sát giờ khóa cược")
                            status=maybe_place_real_bet(conn,info["issue"],gate,args,real_executor,
                                                        submit_guard=current_issue_guard)
